  ...
  implements MessagePackDecoder<TBuffer>
{
  ...
  decodeControlSlow(headerByte: number): unknown {
    ...
    switch (headerByte) {
      ...
      case Symbols.FIXEXT1: {
        ...
        return this.decodeExtension(extensionId, 1);
      }
      case Symbols.FIXEXT2: {
        const extensionId = this.view.getInt8(this.offset++);

        return this.decodeExtension(extensionId, 2);
      }
      case Symbols.FIXEXT4: {
        const extensionId = this.view.getInt8(this.offset++);

        return this.decodeExtension(extensionId, 4);
      }
      case Symbols.FIXEXT8: {
        const extensionId = this.view.getInt8(this.offset++);

        return this.decodeExtension(extensionId, 8);
      }
      case Symbols.FIXEXT16: {
        const extensionId = this.view.getInt8(this.offset++);

        return this.decodeExtension(extensionId, 16);
      }
      case Symbols.STR8: {
        const byteLength = buffer[this.offset++];
  ...
  decode<TValue = unknown>(encodedBuffer: TBuffer): TValue {
    this.setBuffer(encodedBuffer);

    return this.nextValue<TValue>();
  }
}
